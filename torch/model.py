...
class RNNInput(torch.nn.Module):
    def __init__(self, vocab_embedding_params, queries):
        super().__init__()
        self.embedded_query = torch.nn.Embedding(vocab_embedding_params, queries)
        
    def forward(self, x):
        return self.embedded_query(x).mean(dim=-1)
...
class Learner(object):
    ...
    def _random_uniform_unit(self, r, c):
        ...
        init_matrix = np.array([row / np.linalg.norm(row) for row in init_matrix])
    ...
    def _clip_if_not_None(self, g, v, low, high):
        ...
        if g is not None:
            return (torch.clip(g, low, high), v)
        else:
            return (g, v)
    
    def _build_input(self):
        self.tails = torch.empty(0, dtype=torch.int32)  # 또는 torch.tensor([], dtype=torch.int32)
        self.heads = torch.empty(0, dtype=torch.int32)
        
        # one-hot encoding
        def one_hot_targets(heads, num_entity):
            return F.one_hot(heads, num_classes=num_entity)

        self.targets = one_hot_targets(self.heads, self.num_entity)
            
        if not self.query_is_language:
            self.queries = torch.empty(0, self.num_step, dtype=torch.int32) #  tf.placeholder(tf.int32, [None, self.num_step])
            self.query_embedding_params = torch.nn.Parameter(
                       torch.tensor(self._random_uniform_unit(self.num_query + 1, self.query_embed_size), dtype=torch.float32)
            )
        
            rnn_inputs =  torch.nn.Embedding(self.num_query + 1, self.query_embed_size) 
        else:
            self.queries = torch.empty(0, self.num_step, self.num_word, dtype=torch.int32) #   tf.placeholder(tf.int32, [None, self.num_step, self.num_word])
            self.vocab_embedding_params =  torch.nn.Parameter(
                       torch.tensor(self._random_uniform_unit(self.num_vocab + 1, 
                                                              self.query_embed_size), dtype=torch.float32)
            )
            rnn_inputs = RNNInput(self.vocab_embedding_params, self.queries)
    ...
    def _build_graph(self):
        ...
        self.rnn_inputs = [torch.reshape(q, [-1, self.query_embed_size]) 
                           for q in torch.split(rnn_inputs, 
                                             self.num_step, 
                                             dim=1)]
        ...
        self.cell = tf.contrib.rnn.core_rnn_cell.MultiRNNCell(
                                                    [cell] * self.num_layer, 
                                                    state_is_tuple=True)
        init_state = torch.zeros(torch.shape(self.tails)[0], torch.float32)
